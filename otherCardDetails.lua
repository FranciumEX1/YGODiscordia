...
function otherStats.getName(cardName)
    ...
    local searcher = conn:exec("SELECT Cardname FROM SpellsAndTraps WHERE Cardname = '" .. cardName .. "'")
...
function otherStats.getDescription(cardName)
    local conn = dbase.open("YuGiscord.db")
    local searcher = conn:exec("SELECT Description FROM SpellsAndTraps WHERE Cardname = '" .. cardName .. "'")
    local result = searcher[1][1]
    conn:close()
    return result
end
...
function otherStats.getImage(cardName)
    local conn = dbase.open("YuGiscord.db")
    local searcher = conn:exec("SELECT CardImage FROM SpellsAndTraps WHERE Cardname = '" .. cardName .. "'")
    local result = searcher[1][1]
    conn:close()
    return result
end
